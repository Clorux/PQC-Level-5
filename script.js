...
// ===
// == FUNGSI KRIPTOGRAFI INTI (V1.2 - DIPERBAIKI)
// ===
...
async function generateHybridKemKeys() {
    ...
    // PERBAIKAN V1.2: Menambahkan keyUsage yang hilang.
    const classicPair = await window.crypto.subtle.generateKey(
        { name: "ECDH", namedCurve: "P-521" },
        true,
        ['deriveKey', 'deriveBits'] // Di V1.1, ini hilang dan akan menyebabkan kegagalan.
    );
...
async function generateHybridSignKeys() {
    ...
    const verifyPublicKey = {
        ...
        // PERBAIKAN V1.2: Memperbaiki tipo 'midsa' menjadi 'mldsa'.
...
async function hybridEncryptAndSign(plaintext, recipientKemKeyJson, senderSignKeyJson) {
    ...
    // --- PERBAIKAN KRITIS V1.2 DIMULAI DI SINI ---
    // BUG FATAL di V1.1 adalah '521'.
    // Nilai yang benar adalah '528' (66 byte), seperti yang dikonfirmasi oleh.[2, 3, 4]
    const ecdhSecret = await window.crypto.subtle.deriveBits(
        { name: "ECDH", public: recipientEcdhKemPubCryptoKey },
        ephemeralEcdhPair.privateKey,
        528 // DIUBAH DARI 521
    );
    // --- PERBAIKAN KRITIS V1.2 SELESAI DI SINI ---
    ...
    const mldsaSignature = ml_dsa87.sign(new Uint8Array(ciphertext),
        senderMlDsaSignPrivUint8);
...
*/
async function hybridVerifyAndDecrypt(payloadBase64, senderVerifyKeyJson,
    recipientKemKeyJson) {
    ...
    const recipientEcdhKemPrivCryptoKey = await window.crypto.subtle.importKey(
        "jwk", recipientKemKey.ecdh, { name: "ECDH", namedCurve: "P-521" }, true, ['deriveKey', 'deriveBits']
    );
    // PERBAIKAN V1.2: Memperbaiki tipo 'MIKem' menjadi 'MlKem'.
...
// ===
// == LOGIKA UI ASLI (TIDAK BERUBAH DARI V1.1)
// ===
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const base64ToUint8Array = (base64) => Uint8Array.from(atob(base64), c =>
        c.charCodeAt(0));
    ...
    generateKemBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            [kemPublicKeyEl, kemPrivateKeyEl].forEach(el =>
                el.value = `ERROR: ${error.message}`);
        } finally {
    ...
    encryptBtn.addEventListener('click', async () => {
        ...
        try {
            const resultB64 = await hybridEncryptAndSign(plaintext, recipientKemKey,
                senderSignKey);
    ...
    decryptBtn.addEventListener('click', async () => {
        ...
        try {
            const decryptedText = await hybridVerifyAndDecrypt(ciphertext, senderVerifyKey,
                recipientKemKey);
